...
class TestGeneralFeatures:
    ...
    def test_get_account_balance(self, client, test_db, mock_redis):
        with patch('simplebank.utils.redis_cache.get_redis_client', return_value=mock_redis):
            ...
            account = next((acc for acc in accounts if acc["balance"] == 500.0), None)
    ...
    def test_transfer_money(self, client, test_db, mock_redis):
        with patch('simplebank.utils.redis_cache.get_redis_client', return_value=mock_redis):
            ...
            account1 = next((acc for acc in accounts1 if acc["balance"] == 1000.0), None)
            account2 = next((acc for acc in accounts2 if acc["balance"] == 500.0), None)
            assert account1 is not None and account2 is not None, "Accounts should be created"
    ...
    def test_insufficient_funds(self, client, test_db, mock_redis):
        with patch('simplebank.utils.redis_cache.get_redis_client', return_value=mock_redis):
            ...
            accounts2 = accounts2_response.json()
            # Find the accounts we just created
            account1 = next((acc for acc in accounts1 if acc["balance"] == 50.0), None)
            account2 = next((acc for acc in accounts2 if acc["balance"] == 100.0), None)
            assert account1 is not None and account2 is not None, "Accounts should be created"
            account1_id = account1["id"]
            account2_id = account2["id"]
